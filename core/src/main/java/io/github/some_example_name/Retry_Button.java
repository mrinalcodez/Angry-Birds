...
public class Retry_Button implements Button_Interface, Serializable {
    ...
    public Retry_Button(String path, int x, int y, int width, int height, float size, SerializableRenderer renderer, SerializableInputMultiplexer multiplexer){
        this.size = size;
        ...
        inputAdapter = new CustomInputAdapter("Button", game){
            ...
            public boolean touchDown(int screenX, int screenY, int pointer, int b) {
                ...
                if(isButtonVisible && button.contains(worldcoordinates.x, worldcoordinates.y)){
                    ...
                    Screen l = new Level(game, level.worldwidth, level.worldheight, level.level);
                    game.setScreen(l);
                    return true;
                }
                return false;
